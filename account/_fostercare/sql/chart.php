...
if (isset($_POST['getChartData'])) {
    ...
    $year = $_POST['year'];
    $fosterhome_id = $_POST['fhid'];

    $stmt = $db->prepare('SELECT chFname, chCreated FROM children WHERE YEAR(chCreated) = ? AND chFosterhomeId = ?');
    $stmt->execute([$year, $fosterhome_id]);
    $dates = $stmt->fetchAll();
...
if (isset($_POST['getChartDataParent'])) {
    ...
    $stmt = $db->prepare('SELECT fpCreated FROM fosterparent WHERE YEAR(fpCreated) = ? AND fpFosterparentId = ?');
    $stmt->execute([$year, $fosterhome_id]);
    $dates = $stmt->fetchAll();
